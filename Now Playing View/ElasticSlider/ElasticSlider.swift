...
private extension Comparable {
    @inline(__always)
    func es_clamped(to limits: ClosedRange<Self>) -> Self {
...
private extension ElasticSlider {
        ...
        GeometryReader { proxy in
            ...
            .highPriorityGesture(
                DragGesture(minimumDistance: 0)
                    ...
                    .onChanged { g in
                        let progress = (g.translation.width / trackWidth(for: size.width, active: true))
                        * range.distance
                        + lastStoredValue
                        self.value = Double(progress).es_clamped(to: range)
    ...
    var normalizedStretchingValue: CGFloat {
        ...
        return stretchingValue.es_clamped(to: -max ... max) / max
    }
...
extension ElasticSlider where LeadingContent == EmptyView {
    init(value: Binding<Double>, in range: ClosedRange<Double>, trailingLabel: (() -> TrailingContent)? = nil) {
        _value = value
        self.range = range
        lastStoredValue = value.wrappedValue
        leadingLabel = nil
...
extension ElasticSlider where TrailingContent == EmptyView {
    init(value: Binding<Double>, in range: ClosedRange<Double>, leadingLabel: (() -> LeadingContent)? = nil) {
        _value = value
        self.range = range
        lastStoredValue = value.wrappedValue
        self.leadingLabel = leadingLabel?()
...
private struct ElasticSliderConfigEnvironmentKey: EnvironmentKey { static let defaultValue: ElasticSliderConfig = .init() }
...
// local size preference for masking widths
